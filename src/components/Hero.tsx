...
const Hero = () => {
  ...
  return (
    <section className={`relative min-h-screen flex items-center justify-center overflow-hidden transition-colors duration-500 ${
      isDark ? 'bg-[hsl(220,20%,6%)]' : 'bg-primary'
    }`}>
      {/* Subtle grid pattern */}
      ...
      <div className={`absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full blur-3xl animate-pulse ${
        isDark ? 'bg-primary/5' : 'bg-white/5'
      ...
      <div className="container mx-auto px-6 relative z-10 text-center">
        <div className="max-w-4xl mx-auto space-y-8">
          {/* Tagline */}
          <p className={`font-body text-sm tracking-[0.3em] uppercase opacity-0 animate-fade-in ${
          ...
          {/* Subheadline */}
          <p className={`font-body text-lg md:text-xl max-w-2xl mx-auto leading-relaxed opacity-0 animate-fade-in ${
            isDark ? 'text-gray-muted' : 'text-white/80'
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 opacity-0 animate-fade-in" style={{ animationDelay: '0.8s' }}>
            <Button 
              variant="hero" 
              size="xl" 
              className={isDark ? '' : 'bg-white text-primary hover:bg-white/90 shadow-lg'}
            >
          ...
          <div className="grid grid-cols-3 gap-8 pt-16 max-w-2xl mx-auto opacity-0 animate-fade-in" style={{ animationDelay: '1s' }}>
            <div className="text-center">
              <p className={`font-body text-4xl md:text-5xl font-bold ${isDark ? 'text-primary' : 'text-white'}`}>$500M+</p>
              <p className={`font-body text-sm mt-2 tracking-wide ${isDark ? 'text-gray-muted' : 'text-white/70'}`}>Assets Under Management</p>
            </div>
            <div className="text-center">
              <p className={`font-body text-4xl md:text-5xl font-bold ${isDark ? 'text-primary' : 'text-white'}`}>45+</p>
              <p className={`font-body text-sm mt-2 tracking-wide ${isDark ? 'text-gray-muted' : 'text-white/70'}`}>Portfolio Companies</p>
            </div>
            <div className="text-center">
              <p className={`font-body text-4xl md:text-5xl font-bold ${isDark ? 'text-primary' : 'text-white'}`}>12</p>
              <p className={`font-body text-sm mt-2 tracking-wide ${isDark ? 'text-gray-muted' : 'text-white/70'}`}>Unicorn Exits</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t to-transparent ${
        isDark ? 'from-[hsl(220,20%,6%)]' : 'from-white'
      }`} />
    </section>
  );
};
